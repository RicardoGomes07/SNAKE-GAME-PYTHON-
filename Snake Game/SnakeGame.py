...
class SNAKE:
    ...
    def snake_head(self):
        head_relat = self.body[1] - self.body[0]
        if head_relat == Vector2(1,0): self.head = self.head_left
        if head_relat == Vector2(-1,0): self.head = self.head_right
        if head_relat == Vector2(0,1): self.head = self.head_up
        if head_relat == Vector2(0,-1): self.head = self.head_down

    def snake_tail(self):
        tail_relat = self.body[-2] - self.body[-1]
        if tail_relat == Vector2(1,0): self.tail = self.tail_left
        if tail_relat == Vector2(-1,0): self.tail = self.tail_right
        if tail_relat == Vector2(0,1): self.tail = self.tail_up
        if tail_relat == Vector2(0,-1): self.tail = self.tail_down
...
apple = pygame.image.load('imagens/apple.png').convert_alpha()
